...
```
```luau
bolt.create_mesh(part: MeshPart | UnionOperation, convex_radius: number?): MeshShape
```
...
:::
:::caution
Only convex meshes are supported. If you set the CollisionFidelity of a mesh to Hull or Box, MPR and GJK will be able to use it without issues.
:::